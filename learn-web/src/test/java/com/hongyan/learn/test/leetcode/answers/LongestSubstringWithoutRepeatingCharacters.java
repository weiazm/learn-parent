...
public class LongestSubstringWithoutRepeatingCharacters {
    ...
    public int lengthOfLongestSubstring(String s) {
        ...
        /*
         * int len = s.length(); int max = 0; ArrayList<Character> list ; for(int i = 0; i < len; i++) { list = new
         * ArrayList<Character>(); int j; for(j = i; j < len; j++) { char c = s.charAt(j); if(list.contains(c)) break;
         * else list.add(c); } max = Math.max(max, j - i); } return max;
         */
