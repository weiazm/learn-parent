...
public class PartitionList {
     ...
     * ArrayList<Integer> small = new ArrayList<Integer>(); ArrayList<Integer> large = new ArrayList<Integer>();
     * ListNode cur = head; while(cur != null) { if(cur.val < x) small.add(cur.val); else large.add(cur.val); cur =
     * cur.next; } cur = head; int i = 0, j = 0; while(cur != null) { if(i < small.size()) { cur.val = small.get(i);
     * i++; } else { cur.val = large.get(j); j++; } cur = cur.next; } return head; }
     */
