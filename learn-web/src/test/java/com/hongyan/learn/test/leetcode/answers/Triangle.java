package com.hongyan.learn.test.leetcode.answers;/*
                                                 ...
                                                 * bottom is 11 (i.e., 2 + 3 + 5 + 1 = 11). Note: Bonus point if you are
                                                 * able to do this using only O(n) extra space, where n is the total
    ...
    public int minimumTotal(List<List<Integer>> triangle) {
        ...
        for (int i = xLen - 1; i >= 0; i--) {
            ...
            for (int j = 0; j < yLen; j++) {
                dp[j] = (i == xLen - 1 ? 0 : Math.min(dp[j], dp[j + 1])) + triangle.get(i).get(j);
            }
        }
        return dp[0];
    }
