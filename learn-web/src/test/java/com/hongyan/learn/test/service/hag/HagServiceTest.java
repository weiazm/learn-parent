...
public class HagServiceTest {
    ...
    /*
    通过小流量客户端，创建角色、资源，创建的角色和资源名称示例：
    train_role_{your name}{index}, train_resource{your_name}_{index} ，your_name为你的姓名全拼，index从1开始
    通过小流量客户端，获取并打印你创建的角色和资源列表，并判断某个测试的实体key是否属于某个角色或者某个名单
    例如创建了一个姓名名单，则测试某个姓名是否在该名单
    hag.token.id=1000003
hag.token=LnbXTykF7XspVUSAVvEj
     */
    @Value("${hag.app}")
    String app;
